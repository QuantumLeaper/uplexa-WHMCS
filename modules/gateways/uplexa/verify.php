...
function verify_payment($payment_id, $amount, $amount_upx, $invoice_id, $fee, $status, $gatewaymodule, $hash, $secretKey, $currency){
	...
	if ($payment_id !="") {
		...
		if ($hash != md5($invoice_id . $payment_id . $amount_upx . $secretKey)) {
		...
		// send each uplexa tx in the mempool to handle_whmcs
		if ($check_mempool) {
			$get_payments_method = $uplexa_daemon->get_transfers('pool', true);
			foreach ($get_payments_method["pool"] as $tx => $transactions) {
				...
				if(isset($txn_amt)) {
					return handle_whmcs($invoice_id, $amount_upx, $txn_amt, $txn_txid, $txn_payment_id, $payment_id, $currency, $gatewaymodule);
				}
			}
		}
		// send each uplexa tx to handle_whmcs
...
function handle_whmcs($invoice_id, $amount_upx, $txn_amt, $txn_txid, $txn_payment_id, $payment_id, $currency, $gatewaymodule) {
	$amount_atomic_units = $amount_upx * 100;

	//check if uplexa tx already exists in whmcs
	...
	if ($txn_payment_id == $payment_id) {
		if (!$transaction_exists) {
			...
			$fiat_paid = upx_to_fiat($txn_amt, $currency);
		...
		if ($txn_amt * 1.02 >= $amount_atomic_units) {
			...
			return "Error: Amount " . $txn_amt / 100 . " UPX too small. Please send full amount or contact customer service. Transaction ID: " . $txn_txid . ". Payment ID: " . $payment_id;
		}
	}
}


function add_payment($command, $invoice_id, $txn_txid, $gatewaymodule, $fiat_paid, $amount_upx, $payment_id, $fee) {
	$postData = array(
		'action' => $command,
...
$vefiry = verify_payment($payment_id, $amount, $amount_upx, $invoice_id, $fee, $status, $gatewaymodule, $hash, $secretKey, $currency);
echo $vefiry;
