 ...
 * http://implix.com
 * Modified to work with uplexa-rpc wallet by Serhack and cryptochangements
 */
...
class uPlexa_rpc
{
    ...
    public function __construct($pUrl, $pUser = null, $pPass = null) {
		...
		$gatewayx = getGatewayVariables("uplexa");
  ...
  /*  public function setParametersStructure($pParametersStructure)
   ...
   private function request($pMethod, $pParams)
        ...
        if (isset($responseDecoded['error']))
        {
            $errorMessage = 'Request have return error: ' . $responseDecoded['error']['message'] . '; ' . "\n" .
                ...
                if ($responseDecoded['error']['message'] == "Method not found") {
                	$errorMessage .= " Check the daemon hostname and daemon port settings in the uPlexa Pyament Gateway WHMCS config.";
                }
    ...
    public function make_uri($address, $amount_upx, $recipient_name = null, $description = null)
    {
        // If I pass 1, it will be 0.0000001 upx. Then 
        $new_amount = $amount_upx * 100000000;
    ...
    public function transfer($amount_upx, $address, $mixin = 4)
    {
        $new_amount = $amount_upx  * 1000000000000;
